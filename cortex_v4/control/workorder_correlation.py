...
def correlation_from_broker(raw: Mapping[str, Any], order: WorkOrder) -> BrokerCorrelation:
    """Map an authorized broker WorkOrder envelope into Cortex correlation facts.
    ...
    Unknown broker fields are deliberately ignored rather than becoming Cortex policy.
    """
    if not isinstance(raw, Mapping):
