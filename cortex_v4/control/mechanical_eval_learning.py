"""Fifth-loop mechanical eval/learning migration gate (M4/M5/M9/M12/M16/M17/M19/M20/M24).

Binds the eval/learning slice to the V4 controller as runtime checks rather than
prompt wording. Mechanically:

  - selects and records the eval/learning methodology IDs;
  ...
  - runs the eval/learning origin-to-frontier chain (oracle -> calibration -> metric
    -> holdout -> blocked -> refute -> convenience -> qagate) through the strict
    behavioral oracle;
...
Deterministic, public-fixture-only, no provider spend.
"""
...
from ..operation.controllers import run_eval_learning_chain
...
EVAL_LEARNING_METHODOLOGY_IDS = ("M4", "M5", "M9", "M12", "M16", "M17", "M19", "M20", "M24")
TASK_CLASS = "eval-learning-slice"

NAMED_CALLER = f"{NAMED_CALLER_MODULE}.{EVAL_LEARNING_FUNCTION}"
GOVERNING_CALLER = (
    "cortex_v4.control.mechanical_eval_learning.MechanicalEvalLearningController"
)

REQUIRED_CORPUS_REFERENCE = "docs/methodology/WORK-METHODOLOGIES.md"
RECEIPT_SCHEMA = "cortex.loop_engineering.eval_learning_receipt.v1"
...
def assert_eval_path_allowed(path: str | Path, *, label: str = "path") -> Path:
    raw = str(path)
...
def classify_eval(
    ...
    contract_ids = (
        ...
        if contract
        else list(EVAL_LEARNING_METHODOLOGY_IDS)
    )
    methodology_ids: list[str] = []
    for mid in list(contract_ids) + list(EVAL_LEARNING_METHODOLOGY_IDS):
...
def _read_contract(public_dir: Path) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise EvalGateError("CONTRACT_MISSING", f"migration contract missing: {path}")
...
class MechanicalEvalLearningController:
    def __init__(
        self,
        *,
        ssc_root: str | Path,
        ...
        self.public_dir = assert_eval_path_allowed(public_dir, label="public_dir")
    ...
    def select_methodology(self, contract: Mapping[str, Any] | None = None) -> dict[str, Any]:
        classification = classify_eval(contract=contract)
    ...
    def read_corpus(self, ref: str = REQUIRED_CORPUS_REFERENCE) -> dict[str, Any]:
        assert_eval_path_allowed(ref, label="corpus_ref")
        try:
    ...
    def require_named_caller(self) -> None:
        if not NAMED_CALLER:
            raise EvalGateError("NAMED_CALLER_REQUIRED", "no named caller registered")
        self._gate(
            "named_caller",
            True,
    ...
    def run_eval_chain(self, *, disable: tuple[str, ...] = ()) -> dict[str, Any]:
        self.require_named_caller()
        wire = run_eval_learning_chain(
        ...
        cg = call_graph_oracle(NAMED_CALLER_MODULE, EVAL_LEARNING_FUNCTION)
        self.call_graph = cg
        self._gate(
            "call_graph_wire",
            ok=bool(cg.get("ok")),
    ...
    def _build_receipt(self) -> dict[str, Any]:
        ...
        receipt = {
            ...
            "work_unit_id": f"loop-engineering-eval-{uuid.uuid4().hex[:12]}",
            "stage": self._stage,
    ...
    def refuse_closeout_without_receipts(self, receipt: Mapping[str, Any] | None) -> None:
        if not receipt or receipt.get("schema") != RECEIPT_SCHEMA:
            raise EvalGateError(
                "RECEIPT_INCOMPLETE",
                "no eval/learning receipt minted (structured, not prose)",
            )
        if not receipt.get("wire_oracle_ok"):
            raise EvalGateError("RECEIPT_INCOMPLETE", "wire oracle must be ok")
        if not receipt.get("call_graph_wire_ok"):
            raise EvalGateError("RECEIPT_INCOMPLETE", "call-graph wire must be ok")
    ...
    def _refuse_prose_receipt(self, claim: Any) -> None:
        if not isinstance(claim, Mapping) or claim.get("schema") != RECEIPT_SCHEMA:
            raise EvalGateError(
                ...
                "a prose instruction was substituted for structured eval receipt data",
            )

    MUTANT_SPECS = (
        {"id": "EV-wire-rung-dropped", "kind": "wire", "disable": ("calibration",)},
        {"id": "EV-methodology-bypass", "kind": "methodology"},
        {"id": "EV-citation-omitted", "kind": "citation"},
        {"id": "EV-hidden-holdout-exposed", "kind": "hidden"},
        {"id": "EV-callgraph-deleted-caller", "kind": "callgraph"},
        {"id": "EV-closeout-receipt-omitted", "kind": "receipt"},
        {"id": "EV-prose-receipt-substituted", "kind": "prose"},
    )

    def _fresh_good(self) -> "MechanicalEvalLearningController":
        fresh = MechanicalEvalLearningController(
        ...
        for spec in self.MUTANT_SPECS:
            ...
            try:
                if kind == "wire":
                    ...
                    fresh.run_eval_chain(disable=tuple(spec.get("disable") or ()))
                    ...
                    self._refuse_prose_receipt(
                        {"prose": "trust me, the eval slice is done and wired"}
                    )
                    code = "prose_receipt_not_refused"
                else:
                    raise EvalGateError("UNKNOWN_MUTANT", f"unknown kind {kind}")
...
def run_mechanical_eval_learning(
    *,
    ssc_root: str | Path,
    public_dir: str | Path,
    v4_root: str | Path | None = None,
) -> EvalRunResult:
    return MechanicalEvalLearningController(
